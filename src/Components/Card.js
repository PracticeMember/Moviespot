...
export const Card = ({ movies }) => {
  const { overview, title, poster_path, id } = movies
  const poster = poster_path ? `https://image.tmdb.org/t/p/w500/${poster_path}` : PosterNotFound;
  return (

    <div className="max-w-sm bg-gray-200 border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 m-2 ">
      <Link to={`/movie/${id}`}>
